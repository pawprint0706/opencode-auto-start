...
삭제하려면 같은 파일을 실행한 뒤 `2`를, 업데이트 및 재시작은 `3`을, 자동 업데이트 설정 변경은 `4`를, 종료는 `5`를 선택합니다. 각 작업 후에는 메뉴로 돌아가며 `5`를 선택해야 콘솔이 닫힙니다. `3`은 서버 재시작 전에 opencode를 최신 버전으로 업데이트합니다. 삭제 시 LaunchAgent·Finder 빠른 동작·래퍼·저장 비밀번호가 함께 제거됩니다.
